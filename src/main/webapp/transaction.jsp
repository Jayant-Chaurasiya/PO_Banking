...
List <TransactionLog > transactionlist=(List <TransactionLog>)session.getAttribute("transactionlist");
%>
...
<h2>Customer details</h2>
	...
	<table cellpadding="10" cellspacing="10">
		...
		<th>Transfer To Customer Id</th>
		
		<th>Comment</th>
		<%
		for (int i = 0; i < transactionlist.size(); i++) {
			TransactionLog transactions = transactionlist.get(i);
		%>
		<tr>
			<td><%= transactions.getTransactionId() %></td>
			<td><%= transactions.getCustomerId() %></td>
			<td><%= transactions.getDepositedAmount() %></td>
			<td><%= transactions.getWithdrawnAmount() %></td>
			<td><%= transactions.getTransferredAmount() %></td>
			<td><%= transactions.getTransferToCustId() %></td>
			<td><%= transactions.getComment() %></td>
